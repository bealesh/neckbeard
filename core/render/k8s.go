...
// Kustomization CRs per env and CI-authored, reviewable digest bumps for
// stg/prd (DESIGN §11.2).
//
...
func k8sDelivery(bp *blueprint.Blueprint, cfg k8sLaneConfig) []ownership.File {
	...
	for _, env := range bp.Environments {
		...
		files = append(files,
			ownership.File{Path: dir + "/kustomization.yaml", Content: []byte("apiVersion: kustomize.config.k8s.io/v1beta1\nkind: Kustomization\nresources:\n  - flux-system\n  - controllers.yaml\n  - release-source.yaml\n  - apps.yaml\n"), Owner: ownership.OwnerUser},
			ownership.File{Path: dir + "/release-source.yaml", Content: fluxReleaseSourceYAML(bp), Owner: ownership.OwnerGenerated},
...
// CI release records and promotion PRs own the real digests.
...
func containerYAML(bp *blueprint.Blueprint, s blueprint.Service, indent string) string {
	...
	w("- name: %s", s.Name)
	// The user-owned releases/<env> overlay owns the immutable image pin.
	...
	hasSecrets := false
	for _, env := range bp.Environments {
		hasSecrets = hasSecrets || len(secretNames(env)) > 0
	}
	w("  envFrom:")
	w("    - configMapRef:")
	w("        name: %s-config", bp.App)
	if hasSecrets {
		w("    - secretRef:")
...
spec:
  ...
  timeout: 3m
  wait: true
`, generatedYAMLManifestHeader, env, bp.App, bp.App, env, prune, bp.App)
}

func fluxReleaseSourceYAML(bp *blueprint.Blueprint) []byte {
	host := map[string]string{"github": "github.com", "gitlab": "gitlab.com"}[bp.VCS]
	return fmt.Appendf(nil, `%s# The approved release job owns ref.commit after this resource is created.
# IfNotPresent prevents the parent Flux reconciliation resetting that pin.
# The initial invalid commit deliberately prevents a first unapproved deploy.
...
spec:
  ...
  secretRef:
    name: flux-system
`, generatedYAMLManifestHeader, bp.App, "ssh://git@"+host+"/"+bp.Repo)
}
